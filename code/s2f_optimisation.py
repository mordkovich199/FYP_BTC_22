...
from models import optimize_s2f_new, r_squared, update_progress
...
def predict_s2f(inputs, factors):
    f1 = factors['f1']
    f2 = factors['f2']
    out = f1 * pow(inputs['sf'], f2)
    return out
...
def evaluate():
    ...
    for val_size in splits:
        ...
        for i in range(N_JOBS):
            ...
            factors = optimize_s2f_new(x_train, y_train, grid)
            split_factors_list.append(factors)

            #Training set performance
            train_rmse = mean_squared_error(y_train, predict_s2f(x_train, factors))
            split_train_rmse_list.append(train_rmse)
            train_r2 = r_squared(y_train, predict_s2f(x_train, factors))
            split_train_r2_list.append(train_r2)

            #Validation set performance
            val_rmse = mean_squared_error(y_val, predict_s2f(x_val, factors))
            split_val_rmse_list.append(val_rmse)
            val_r2 = r_squared(y_val, predict_s2f(x_val, factors))
        ...
        test_r2 = r_squared(y_test, predict_s2f(x_test, split_best_factors))
        test_r2s.append(test_r2)
        test_rms = mean_squared_error(y_test, predict_s2f(x_test, split_best_factors))
...
def visualize(mode='simple'):
    ...
    y_predicted_rmse = predict_s2f(x, {'f1': 0.48, 'f2': 3.1})
    y_predicted_r2 = predict_s2f(x, {'f1': 0.48, 'f2': 3.08})
